drop proc dbo.Consulta104
go

CREATE PROCEDURE dbo.Consulta104
(
...
BEGIN
  ...
  Exec Emulador_SepararCadena 'V1', @Cabezero, '|', @Valor1 Output --Codigo Unidad
  ...
  If dbo.Fnc_ESNumero(@Valor1) <> 1
     Begin
       Select @Resul = '2R=ERROR|2M=El codigo de la unidad debe ser numérico|'
       Return
     End
  ...
  If @Validar = 0               
   Begin
     Select Codigo = Str(IsNull(IdUnidad,0)),
            Descripción = IsNull(Descripcion,'')
     From SMercado..Cat_Unidades (NoLock)
     Order By IdUnidad 
  ...
  If @Validar = 3
      Begin
       ...
       Select @Desc1 = Isnull(Min(IsNull(IdUnidad,0)),0)
       From SMercado..Cat_Unidades (NoLock)
       Where Len(RTrim(LTrim(Descripcion))) = 0                   
  
       If @Desc1 = 0
        Begin
          Insert SMercado..Cat_Unidades(Descripcion) 
          Values('')
  ...
  If @Registro = 0
   Begin
      If @Validar = 0 
         Select @Resul = '2R=ERROR|2M=No hay datos en el catálogo de departamentos|'
      If @Validar = 1 
         Select @Resul = '2R=OK|'
      If @Validar = 2 
         Select @Resul = '2R=OK|'
  ...
  Else
    Begin
	  If @Validar = 0 
	     Select @Resul = '2R=OK|CATALOGO=Catálogo de departamentos|'
