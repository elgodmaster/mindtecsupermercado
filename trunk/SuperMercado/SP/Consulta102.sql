drop proc dbo.Consulta102
go

CREATE PROCEDURE dbo.Consulta102
(
...
BEGIN
  ...
  If @Validar = 0               
   Begin
     Select Codigo = Str(IsNull(IDMarca,0)),
  ...
  If @Validar = 2
   ...
   If @Validar = 3
      Begin
       ...
       If @Desc1 = 0
        Begin
          Insert SMercado..Cat_Marcas(Descripcion) 
          Values('')
  ...
  If @Registro = 0
   Begin
      If @Validar = 0 
         Select @Resul = '2R=ERROR|2M=No hay datos en el catálogo de Marcas|'
      If @Validar = 1 
         Select @Resul = '2R=OK|'
      If @Validar = 2 
         Select @Resul = '2R=OK|'
  ...
  Else
    Begin
	  If @Validar = 0 
	     Select @Resul = '2R=OK|CATALOGO=Catálogo de Marcas|'
