drop proc dbo.Reporte006
go

CREATE  PROCEDURE [dbo].[Reporte006]
(
...
BEGIN
  ...
  If @Validar = 1
   Begin 
	   ...
	   --Armar Query
     Select @Sql = " Select C0 = LTrim(RTrim(IsNull(a.nocotizacion,''))), " +
       ...
       "From SMercado..Cotizaciones a " +  
       "Left Join SMercado..Cotizaciones_Detalles b On b.NoCotizacion = a.Nocotizacion " + 
       "Left Join SMercado..Cat_Clientes c On c.Codigo = a.IdCliente " + 
       " Where 1 = 1 " 

             If Len(@Valor1) > 0
              Begin
               Select @Sql = @Sql + " And a.NoCotizacion = '" + @Valor1 + "' "
              End
...
Select @Sql = @Sql + "And a.Fecha Between '" + @Valor3 + "' And '" + @Valor4 + "' "
  ...
  Select @Sql = @Sql + " group by a.NoCotizacion,c.NombreFiscal, a.Fecha  order by a.fecha,c.NombreFiscal"
  ...
  If @Registro = 0
   Begin
	  If @Validar = 1 
         Select @Resul = '2R=ERROR|2M=No hay información de cotizaciones para realizar un reporte'
  End
   Else
    Begin
	  If @Validar = 1
