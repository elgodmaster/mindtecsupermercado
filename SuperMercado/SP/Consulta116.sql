drop proc dbo.Consulta116
go

CREATE  PROCEDURE [dbo].[Consulta116]
(
...
BEGIN
  ...
  If @Validar = 0               ----Catalogo de cuentas
   Begin
 ...
 If @Validar = 2
   Begin
     ...
     Select C1 = IsNull(a.CodigoCliente,''),
            C2 = IsNull(b.NombreFiscal,''),
            C3 = ISNULL(b.RFC,''),
            C4 = ISNULL(a.Fecha,''),
            C5 = ISNULL(a.Adeudo,0)
     From SMercado..Cuentas_Cobrar  a (NoLock)
     Inner Join SMercado..Cat_Clientes b (NoLock) On b.Codigo = a.CodigoCliente 
  ...
  If @Validar = 5
     Begin    
       Select @Desc1 = Isnull(Min(IsNull(IdCuenta,0)),0)
       From SMercado..Cuentas_Cobrar (NoLock)
       Where IdTipoCambio = 10000 And IdUsuario = @Valor1              
  
       If @Desc1 = 0
        Begin
          Insert SMercado..Cuentas_Cobrar(Factura,Fecha,CodigoCliente,IdUsuario,idTipoCambio) 
